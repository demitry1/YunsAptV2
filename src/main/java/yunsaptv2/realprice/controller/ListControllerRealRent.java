...
import yunsaptv2.realprice.service.RealPriceServiceRent;

@WebServlet("/realprice/realpricerent")
public class ListControllerRealRent extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String kind_ = request.getParameter("kind");				
...
//		String date = sdf.format(cal.getTime());
		...
		RealPriceServiceRent service = new RealPriceServiceRent();
//      실거래가 마스터 리스트가져오기		
		List<RealPriceRent> list = null;  //객체
		try {
			list = service.getRealPriceListRent
					(city_do, city_gu, dong_gu, scale_beg, scale_end, contract_date1, contract_date2, 
					 construction_year_beg, construction_year_end, apt_name, page, cmd, realall, listcnt, kind);
...
//      실거래가 마스터 총 카운트 가져오기	
		...
		try {
			count = service.getRealPriceCountRent(city_do, city_gu, dong_gu, scale_beg, scale_end, 
					 contract_date1, contract_date2, construction_year_beg, construction_year_end, 
					 apt_name, cmd, realall, kind);
		...
		request.getRequestDispatcher("/WEB-INF/view/realprice/realprice_rent.jsp").forward(request, response);
	}
